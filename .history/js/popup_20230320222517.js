...
chrome.storage.local.get(['dark_mode', 'dyslexia_mode', 'cb_mode'], function(result){
    ...
    document.querySelector('#cb_mode').checked = result.cb_mode;
})
switches.forEach(function (option) {
    ...
    document.querySelector('#' + option + ' > .slider').addEventListener('mouseup', function () {
        ...
        switch (option) {
            ...
            case 'cb_mode': chrome.storage.local.set({ cb_mode1: status }); sendFromPopup("colorblindmode"); break;
        }
    });
});
