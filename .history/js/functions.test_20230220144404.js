...
test('Toggle switch for dark mode appears', () => {
  expect(document.querySelector('#dark-mode-toggle')).not.toBeNull();
});
...
test('Hyperlink color changes to light blue in dark mode', () => {
  ...
  // toggle.click();
  expect(getComputedStyle(document.querySelector(background.hyperlinkSelector)).color).toBe('rgb(92, 165, 246)');
});

// Test that the color of hyperlinks changes back to red when dark mode is deactivated.
test('Hyperlink color changes back to red in light mode', () => {
  const toggle = document.querySelector('#dark-mode-toggle');
  toggle.click();
